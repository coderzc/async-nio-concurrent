...
final class PlatformDependent0 {
    ...
    private static Throwable explicitNoUnsafeCause0() {
        final boolean noUnsafe = SystemPropertyUtil.getBoolean("io.io.netty.noUnsafe", false);
        logger.debug("-Dio.io.netty.noUnsafe: {}", noUnsafe);

        if (noUnsafe) {
            logger.debug("sun.misc.Unsafe: unavailable (io.io.netty.noUnsafe)");
            return new UnsupportedOperationException("sun.misc.Unsafe: unavailable (io.io.netty.noUnsafe)");
        }

        // Legacy properties
        String unsafePropName;
        if (SystemPropertyUtil.contains("io.io.netty.tryUnsafe")) {
            unsafePropName = "io.io.netty.tryUnsafe";
        } else {
            unsafePropName = "org.jboss.io.netty.tryUnsafe";
        }
    ...
    private static boolean explicitTryReflectionSetAccessible0() {
        ...
        return SystemPropertyUtil.getBoolean("io.io.netty.tryReflectionSetAccessible", javaVersion() < 9);
    }
