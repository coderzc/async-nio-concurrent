...
/**
 * Tells if <a href="http://netty.io/wiki/forked-tomcat-native.html">{@code io.netty-tcnative}</a> and its OpenSSL support
 * are available.
 */
public final class OpenSsl {
    ...
    static {
        ...
        if (SystemPropertyUtil.getBoolean("io.io.netty.handler.ssl.noOpenSsl", false)) {
            cause = new UnsupportedOperationException(
                    "OpenSSL was explicit disabled with -Dio.io.netty.handler.ssl.noOpenSsl=true");

            logger.debug(
                    "io.netty-tcnative explicit disabled; " +
                            OpenSslEngine.class.getSimpleName() + " will be unavailable.", cause);
        } else {
            // Test if io.netty-tcnative is in the classpath first.
            try {
                Class.forName("io.netty.internal.tcnative.SSL", false, OpenSsl.class.getClassLoader());
            } catch (ClassNotFoundException t) {
                cause = t;
                logger.debug(
                        "io.netty-tcnative not in the classpath; " +
                                OpenSslEngine.class.getSimpleName() + " will be unavailable.");
            }

            // If in the classpath, try to load the native library and initialize io.netty-tcnative.
            if (cause == null) {
                try {
                    ...
                    cause = t;
                    logger.debug(
                            "Failed to load io.netty-tcnative; " +
                ...
                try {
                    ...
                    logger.debug(
                            "Failed to initialize io.netty-tcnative; " +
                                    OpenSslEngine.class.getSimpleName() + " will be unavailable. " +
                                    "See http://netty.io/wiki/forked-tomcat-native.html for more information.", t);
                }
            }
        ...
        if (cause == null) {
            logger.debug("io.netty-tcnative using native library: {}", SSL.versionString());
            ...
            try {
                ...
                try {
                    ...
                    try {
                        ...
                        try {
                            SSL.setHostNameValidation(ssl, 0, "io.netty.io");
                                ...
                                useKeyManagerFactory = AccessController.doPrivileged(new PrivilegedAction<Boolean>() {
                                    ...
                                    public Boolean run() {
                                        return SystemPropertyUtil.getBoolean(
                                                "io.io.netty.handler.ssl.openssl.useKeyManagerFactory", true);
                                    }
                                });
                            } catch (Throwable ignore) {
     ...
     * <a href="http://netty.io/wiki/forked-tomcat-native.html">{@code io.netty-tcnative}</a> and its OpenSSL support
     * are available.
     */
    ...
    /**
     * Ensure that <a href="http://netty.io/wiki/forked-tomcat-native.html">{@code io.netty-tcnative}</a> and
